 ...
 */
public class JsonStoryFromCat extends DefaultHandler {
...
*/
    ...
    static public void main (String[] args) {
        ...
        JsonStoryFromCat jsonStoryFromCat = new JsonStoryFromCat();

        if (!pathToCatFile.isEmpty()) {
            File catFile = new File (pathToCatFile);
            folder = catFile.getParent();
            jsonStoryFromCat.parseFile(pathToCatFile);
            jsonStoryFromCat.getPhrases();
            jsonStoryFromCat.getActorLinks();
            jsonStoryFromCat.buildEventCorefMap();
            jsonStoryFromCat.buildLinkMaps();
            jsonStoryFromCat.buildTermMaps();
            try {
                jsonStoryFromCat.buildMentionMap();
            } catch (JSONException e) {
                e.printStackTrace();
            }
            jsonStoryFromCat.makeGroupsForInstance();
            System.out.println("jsonFromCat.eventTermArrayList = " + jsonStoryFromCat.eventTermArrayList.size());
            System.out.println("jsonFromCat.eventCorefMap = " + jsonStoryFromCat.eventCorefMap.size());
            ArrayList<JSONObject> events = null;
            try {
                events = jsonStoryFromCat.getJsonObjectsFromCorefsets();
            } catch (JSONException e) {
                e.printStackTrace();
            }
            ...
            for (int i = 0; i < files.size(); i++) {
                ...
                jsonStoryFromCat.parseFile(file.getAbsolutePath());
                jsonStoryFromCat.getPhrases();
                jsonStoryFromCat.getActorLinks();
                jsonStoryFromCat.buildEventCorefMap();
                jsonStoryFromCat.buildLinkMaps();
